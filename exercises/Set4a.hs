...
distinct [] = True
distinct (x:xs) = foldr (\y acc -> y /= x && acc) True xs && distinct xs
...
longest [xs] = xs
longest (x:xs)
    | length x > length lonxs = x
    | length x == length lonxs = if head x <= head lonxs then x else lonxs
    | otherwise = lonxs
  where lonxs = longest xs
...
incrementKey k = map incrementKeySingle
    where incrementKeySingle (j, v) = if k == j then (j, v + 1) else (j, v)
...
freqs = foldr foldFn Map.empty
  where
    helperFn :: Maybe Int -> Maybe Int
    helperFn Nothing = Just 1
    helperFn (Just x) = Just (x + 1)

    foldFn :: Ord a => a -> Map.Map a Int -> Map.Map a Int
    foldFn key map = Map.alter helperFn key map
...
transfer from to amount bank = if conditionsMet then newBank else bank
  where
    newBank = Map.adjust (\x -> x + amount) to . Map.adjust (\x -> x - amount) from $ bank

    conditionsMet = accountsExist && sumIsPositive && sufficientFunds
    accountsExist = Map.member from bank && Map.member to bank
    sumIsPositive = amount > 0
    sufficientFunds = case Map.lookup from bank of Nothing -> False
                                                   Just x -> x >= amount
