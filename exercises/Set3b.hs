...
sums n = sums' 1 n
    where
        sums' :: Int -> Int -> [Int]
        sums' acc 1 = [acc]
        sums' acc k = acc : sums' (acc + n - k + 2) (k - 1)
...
sorted' :: [Int] -> Int -> Bool
sorted' [] _ = True
sorted' (x:xs) last_val = if x < last_val then False else sorted' xs x
...
sumsOf' :: Int -> [Int] -> [Int]
sumsOf' current [] = [current]
sumsOf' current (x:xs) = current : sumsOf' (current + x) xs
